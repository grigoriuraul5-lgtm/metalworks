...
// Sursă unică de adevăr pentru produsele reale "Picioare de Masă" — aceleași
// date alimentează și feed-ul Google Merchant Center (api/google-feed.js).
// Pozele lor stau în /public/products/<slug>.jpg (cale stabilă, necesară
// pentru Google), nu în ./assets ca restul imaginilor din site.
import { KRAFT_PRODUCTS } from './data/kraftFeedProducts.js';
...
const PRODUCTS = [
  ...
  // Generate automat din src/data/kraftFeedProducts.js — adaugă produse noi
  // acolo, nu aici, ca site-ul și feed-ul Google să rămână mereu sincronizate.
  ...KRAFT_PRODUCTS.map((p) => ({
    id: `pmn-${p.slug}`,
    slug: p.slug,
    category: 'picioare-masa',
    isNew: true,
    title: p.title,
    imgRender: `/products/${p.slug}.jpg`,
    aspect: 'landscape',
    hasSketch: false,
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="columns-1 gap-4 sm:columns-2 lg:columns-3">
            {displayedProducts.map((product) => (
              <div
                key={product.id}
                id={product.slug ? `produs-${product.slug}` : undefined}
